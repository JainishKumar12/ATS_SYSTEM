...
# auth state . populated by supabase sign-in/sign-out/oAuth
for key , default in [
    ("access_token", None),
    ("refresh_token", None),
    ("user_id", None),
...
# if we just came back from Google Oauth  , supabase append `?code=<authcode>`
if (
    ...
    # always clear the code= param so a refresh doesnt try to re-exchange the code for a session
...
# Initialize session state for view managment
...
with st.sidebar:
    ...
    if st.session_state.access_token:
        ...
        if st.button("Sign Out", use_container_width=True):
            ...
            for k in ("access_token", "refresh_token", "user_id", "user_email", "auth_error", "auth_info"):
        ...
        with tab_up:
            with st.form("signup_form", clear_on_submit=False):
                ...
                password_up = st.text_input("Password (min 6 characters)", type="password", key="signup_pw")
        ...
        if "error" in oauth:
            st.caption (f"Google sign-in unavialble: {oauth['error']}")
        else:
            st.link_button(
