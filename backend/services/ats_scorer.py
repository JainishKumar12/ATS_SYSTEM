...
def detect_location_info(text: str, nlp: spacy.Language) -> Dict:
    ...
    #method01: spacy NER Named Entity Recognition
    doc = nlp(text)
    for ent in doc.ents:
...
"""ats_scorer.py
    ├── _tier_score()                  → reusable points-by-threshold helper
    ├── detect_location_info()         → find addresses, zip codes, privacy risk
    ├── _calculate_semantic_similarity() → ML meaning comparison
    ├── _skill_matches()               → fast check then ML check
    ├── validate_skills_with_projects() → are skills proven in projects?
    │
    ├── _calc_formatting_score()       → sections, bullets, structure (max 20)
    ├── _calc_keywords_score()         → keywords, skills, JD match (max 25)
    ├── _calc_content_score()          → verbs, achievements, grammar (max 25)
    ├── _calc_skill_validation_score() → proven skills ratio (max 15)
    ├── _calc_ats_compatibility_score() → location, special chars (max 15)
    │
    ├── calculate_overall_score()      → combines all 5 → final 0-100 score
    ├── generate_strengths()           → what's good
    ├── generate_critical_issues()     → what's failing
    └── generate_improvements()        → what to fix"""
