...
# This is called the Singleton Pattern — it means only ONE client object ever gets created.
...
def _try_parse_json(text: str)-> dict |None:
    cleaned = text.strip() # strip markdown code fences if present
    if cleaned.startswith("```json"):
        first_newline = cleaned.index("\n") if "\n" in cleaned else len(cleaned)
        cleaned = cleaned[first_newline + 1:]
        if cleaned.endswith("```"):     # remove closing fence
            cleaned = cleaned[:-3]
        cleaned = cleaned.strip()

    try:
        return json.loads(cleaned)     # json.loads() — converts JSON string to Python dictionary. Returns None if it fails instead of crashing.
