...
def calculate_semantic_similarity(
    ...
    resume_emb = embedder.encode(resume_text[:5000], convert_to_tensor=False)  # [:5000] truncates both texts to 5000 characters before encoding. SentenceTransformer models have a token limit — feeding the full text could silently truncate or error. This is a safe cap. convert_to_tensor=False returns a plain numpy array instead of a PyTorch tensor — needed for np.dot() to work.
...
def analyze_skills_gap(
        resume_skills:List[str], jd_text:List[str] , nlp: spacy.Language
) -> list[str]:
