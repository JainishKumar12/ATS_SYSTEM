# it will tell us , what issue we have to work on first 
from dataclasses import dataclass
from enum import Enum
from typing import Dict , List , Optional

class Priority(Enum):   # An Enum makes priority a fixed set of choices instead of raw strings. So you can never accidentally write Priority.HIGH or "critica" — Python catches it at runtime. Also enables sorting logic later.
    CRITICAL = "Critical"
    HIGH = "high"
    MEDIUM = "medium"
    LOW = "low"
...
def generate_skill_recommendations(skill_validation_results: Dict) -> List[Recommendation]:
    ...
    # Tuple unpacking on the right side — assigns both variables in one line. The thresholds map validation percentage to priority level. Below 40% validated → Critical. Notice impact score scales with priority (8 → 6 → 4 → 2).
    ...
    # [:5] slices to show only first 5 skills — avoids overwhelming the user. The > 5 check appends a count of remaining ones so nothing is silently hidden.
    action_items = [
...
def generate_grammar_recommendations(grammar_results: Dict) -> List[Recommendation]:
    ...
    if critical_errors:
        ...
        for error in critical_errors[:5]:
            ...
            suffix  = f" → '{suggest[0]}'" if suggest else ''   # Conditional f-string — only adds the suggestion arrow if suggestions exist. Prevents output like "Fix 'teh' → : spelling error" when suggestions list is empty.
        ...
        recommendations.append(Recommendation(
            ...
            impact_score = min(10.0, len(critical_errors) * 2.0),  # min(10.0, ...) caps the impact at 10 regardless of how many errors exist. Without this, 10 critical errors would give an impact of 20.0 which breaks the scoring scale.
            category     = 'grammar',
            action_items = items,
        ))
    ...
    if minor_errors and len(minor_errors) >= 3:  # Minor errors only get reported if there are 3 or more — avoids nitpicking 1–2 style issues.
        recommendations.append(Recommendation(
